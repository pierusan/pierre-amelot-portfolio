...
In 2017, 4,564 lives were lost in crashes involving large trucks or buses
...
particular from start to finish. As the _only researcher, designer, and front-end
developer on the project_, I was in charge of creating, almost from scratch, a
real-time lidar point cloud visualization to help drivers see 360° and avoid
collisions.

<ProjectImageWithCaption
  name="fg_surr_system_diagram"
...
the user-centered design process_, from contextual inquiry with truck drivers, to
diary study and product analytics dashboards to understand usage after launch.

<ProjectImageWithCaption
  name="fg_surr_usertest_sprinter"
  caption="The components of Fleetguide 360° Surround View"
/>

At the end of this process, the final design was _a single 3D scene, with 3
camera angles that drivers could customize and toggle between_. A 3D model of the
vehicle was added to the scene, and 2 color palettes were used to differentiate
the ground from other objects.

<ProjectImageWithCaption
  name="fg_surr_camera_views"
  caption="The different camera views of the system"
/>
...
couldn't look everywhere at once, that blind spots remained, and that an enhanced
360° context of their surroundings would help them drive more safely.
...
of them. _I decided to create prototypes directly with code to make use of sample
data I collected with our testing van, and recorded using ROS (The Robot
Operating System)._
...
intuitiveness. _To iterate faster, I extended existing 2D vector (using Cairo) and
3D graphics (using VTK) code meant to run on developer machines, hence avoiding
compatibility and performance requirements of embedded systems for the time
being._
...
At various points, _I asked drivers to rate usefulness of each prototype. A clear
winner emerged: a single top-down view displaying the raw point cloud._ Drivers
explained details of the raw point provided more context than abstracted shapes,
and preferred having a single area of the screen to look at to increase
'glanceability'.

<ProjectImageWithCaption
  name="fg_surr_system_diagram"
  caption="Responses from drivers during the user test"
/>
...
ROS and VTK meant to run on our embedded computer._ The most challenging part was
to find a graphics library that could work on our system (which did not have an
X server). SDL2 handing an OpenGL context to a specific commit of VTK built for
EGL ended up being the solution. The app ran with little latency at ~30FPS, and
was ready to be used in trucks.
...
felt the same in their truck. _I went on multiple ride-alongs with drivers to see
their initial reactions to the real-time display prototype. I watched their
...
vehicles, and see more of the roof of cars. _Using these insights, we adapted our
sensors locations to reduce blind-spots and improve coverage._
...
<ProjectImageWithCaption name="fg_surr_system_diagram" caption="" />
...
in touch with drivers to understand product usage. On top of ride-alongs, I built
SQL Data Studio dashboards displaying product metrics (such as time spent in
each camera view, or time series of the UI state). _I also started a diary study
with drivers via text messages, where I would ask them to text about interesting
or frustrating situations with the display. I also asked them to fill a mobile
survey periodically._
...
<ProjectImageWithCaption
  name="fg_surr_system_diagram"
  caption="Display product roadmap in Airtable"
/>
